...
func migrate0001(db *gorm.DB, name string) error {
	...
	if err = db.CreateTable(new(Confirmation)).Error; err != nil {
		return fmt.Errorf("Error creating confirmations table: %s", err)
	}
	if err = db.CreateTable(new(Invitation)).Error; err != nil {
		return fmt.Errorf("Error creating invitations table: %s", err)
	}
	if err = db.CreateTable(new(PasswordReset)).Error; err != nil {
		return fmt.Errorf("Error creating password_resets table: %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("Error creating foreign key on "+
			"confirmations.user_id for users(id): %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("Error creating foreign key on "+
			"invitations.invited_user_id for users(id): %s", err)
	}
	err = db.Model(new(Invitation)).AddForeignKey(
	...
	if err != nil {
		return fmt.Errorf("Error creating foreign key on "+
			"invitations.invited_by_user_id for users(id): %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("Error creating foreign key on "+
			"password_resets.user_id for users(id): %s", err)
	}
